...
const supportsHyperlinks = typeof stdoutStream !== 'undefined' && typeof stdoutStream.isTTY === 'boolean'
    ? stdoutStream.isTTY
    : false;
...
export function normalizeResponse(payload) {
    ...
    const citations = qa?.citations ||
        ...
        payload?.sources ||
        [];
    const followUps = qa?.follow_up_questions ||
    ...
    const questionAnswerId = qa?.id || qa?.question_answer_id || payload?.question_answer_id;
    return {
        answer: answer || '',
...
export function renderFollowUps(followUps) {
    if (!followUps.length)
        return '';
...
export function formatAnswerBlock(answer) {
    if (!answer)
        return '';
