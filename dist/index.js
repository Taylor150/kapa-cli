...
import { createProfile, deleteProfile, getConfigDirectory, getConfigPath, listProfiles, loadConfig, normalizeConfigKey, resolveProfile, setConfigValue, summarizeProfile, } from './config.js';
import { appendHistory, clearHistory, getHistoryPath, getLastThread, readHistory, getHistoryStatus, } from './history.js';
import { normalizeResponse, renderCitations, renderFollowUps, formatAnswerBlock, } from './format.js';
...
program
    ...
    .option('--metadata <pair...>', 'Attach metadata key=value pairs', collectValues, [])
...
program
    ...
    .action(async (limitOrAction, options) => {
    try {
...
async function handleAsk(promptParts, options) {
    ...
    const apiKey = options.apiKey ?? process.env.KAPA_API_KEY ?? resolved.values.apiKey ?? '';
    const projectId = options.project ?? process.env.KAPA_PROJECT_ID ?? resolved.values.projectId ?? '';
    const integrationId = options.integration ??
    ...
    if (!integrationId) {
        throw new Error('integration_id missing. Provide via --integration or "kapa config set integrationId <value>".');
    }
    const streamPreference = typeof options.stream === 'boolean' ? options.stream : resolved.values.stream;
    const metadata = parseMetadata(options.metadata);
    const temperatureInput = options.temperature !== undefined
        ? Number(options.temperature)
        : resolved.values.temperature ?? undefined;
    const finalTemperature = typeof temperatureInput === 'number' && Number.isFinite(temperatureInput)
        ? temperatureInput
        : undefined;
    ...
    if (!threadId && !projectId) {
        throw new Error('Project id is required to start a new chat. Provide via --project or config.');
    }
    ...
    const onStreamEvent = (event) => {
        if (!streamPreference || !event.text)
            return;
        if (!headerShown) {
            if (spinnerEnabled)
                spinner.stop();
    ...
    if (options.copy) {
        await clipboard
            ...
            .catch(() => process.stderr.write(`${chalk.yellow('!')} Could not access clipboard utility.\n`));
    }
...
async function handleConfig(action, key, value, profile) {
    switch (action) {
        case 'list': {
            const info = await listProfiles();
            for (const [name, profileConfig] of Object.entries(info.profiles)) {
                const summary = summarizeProfile(profileConfig);
                const header = name === info.defaultProfile ? `${chalk.bold(name)} ${chalk.dim('(default)')}` : chalk.bold(name);
                process.stdout.write(`${header}\n`);
                for (const [field, fieldValue] of Object.entries(summary)) {
                    process.stdout.write(`  ${chalk.dim(field.padEnd(14))}${fieldValue ?? chalk.dim('(unset)')}\n`);
                }
                process.stdout.write('\n');
            }
            return;
        ...
        case 'set': {
            ...
            process.stdout.write(`${chalk.green('✓')} Saved ${result.key}${scope}: ${String(result.value)}\n`);
            return;
        }
        case 'get': {
            if (!key)
                throw new Error('Usage: kapa config get <key>');
            ...
            const valueToShow = resolved.values[normalized];
...
async function handleHistory(limitOrAction, jsonOutput) {
...
async function fsWrite(target, content) {
    await writeFile(target, content, 'utf8');
}
void program.parseAsync(process.argv);
function shouldStartInteractiveSession(promptParts, options) {
...
async function startInteractiveSession(options) {
    const config = await loadConfig();
    const resolved = resolveProfile(config, options.profile);
    const baseOptions = { ...options, thread: undefined, resume: undefined };
    let currentThreadId = await resolveInitialThreadId(options, resolved.name);
    let lastAnswer;
    debugLog('startInteractiveSession', JSON.stringify({
        profile: resolved.name,
...
async function resolveInitialThreadId(options, profileName) {
    if (options.thread)
...
const LOGO_WORD = ['K', 'A', 'P', 'A'];
const KAPA_LOGO = LETTER_MAP.K.map((_, row) => LOGO_WORD.map((letter) => LETTER_MAP[letter][row]).join('  '));
...
function gradientColor(colors, ratio) {
    if (colors.length === 1)
        return colors[0];
    ...
    const mixed = start.map((startChannel, idx) => Math.round(startChannel + (end[idx] - startChannel) * localT));
...
function printInteractiveIntro() {
    process.stdout.write('Type your task or use a command:\n\n' +
...
async function handleInteractiveCommand(input, context) {
    ...
    switch (command.toLowerCase()) {
        ...
        case 'cptext': {
            ...
            await clipboard.write(answer).then(() => process.stdout.write(`${chalk.green('✓')} Copied last response to clipboard.\n`)).catch(() => process.stderr.write(`${chalk.yellow('!')} Clipboard unavailable.\n`));
            return true;
        }
        case 'cpcmd': {
            const answer = context.getLastAnswer();
            ...
            await clipboard.write(combined).then(() => process.stdout.write(`${chalk.green('✓')} Copied command block to clipboard.\n`)).catch(() => process.stderr.write(`${chalk.yellow('!')} Clipboard unavailable.\n`));
            return true;
        }
        default:
